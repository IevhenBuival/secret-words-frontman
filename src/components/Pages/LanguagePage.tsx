...
import AddEditLangDlg from "../Dialogs/AddEditLangDlg";
...
const LanguagePage = () => {
  ...
  const { languages, mdbLoading, showMdbLoadingErrors, unAuth, setLanguages } =
    useLanguages(false);
  ...
  return (
    <div>
      ...
      {showMdbLoadingErrors &&
        (unAuth ? (
          <p>U have no rights to access data on this page</p>
        ) : (
      ...
      {!mdbLoading && !showMdbLoadingErrors && (
        <>
          {languages.length > 0 ? (
            <>
              <Button
                className={`mb-4 mt-4 ${styles.blockCenter}`}
                onClick={() => {
                  setShowAddLangDlg(true);
                }}
              >
                <FaPlus className={styles.margin4} />
                Add new language
              </Button>
              {RowGrid}
            </>
      ...
      {langToEdit && (
        <AddEditLangDlg
          ...
          onLangSave={(updLang) => {
            setLanguages(
              languages.map((l) => (l._id === updLang._id ? updLang : l))
            );
            setLangToEdit(null);
          }}
        />
